...
const CouponCard = ({ title, onClick }: CouponCardProps) => {
  ...
  return (
    <div 
      className="w-[161px] md:w-[285px] h-[337px] lg:w-[285px] lg:h-[477px] flex flex-col rounded-[8px] bg-[#070C1B]"
    >
      <div className="h-[285px] bg-[#FFFFFF] rounded-t-[8px] cursor-pointer " onClick={() => handleClickcoupon('1')}>
        <img src="" alt={title} />
      </div>
      <div className="flex flex-col flex-1 items-center gap-y-9 p-4">
        <div className="flex flex-col mt-3 gap-3 items-start">
          <h4 className="headline-4 text-[#FFFFFF] font-bold text-xl line-clamp-2 hover:underline cursor-pointer " onClick={() => handleClickcoupon('1')}>{title}</h4>
