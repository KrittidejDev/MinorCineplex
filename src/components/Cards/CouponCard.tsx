...
const CouponCard = ({ title, onClick }: CouponCardProps) => {
  return (
    ...
    >
      <div className="h-[285px] bg-[#FFFFFF] rounded-t-[8px]" onClick={onClick}>
        <img src="" alt={title} />
      </div>
      <div className="flex flex-col flex-1 items-center gap-y-9 p-4">
        <div className="flex flex-col mt-3 gap-3 items-start">
          <h4 className="headline-4 text-[#FFFFFF] font-bold text-xl line-clamp-2 hover:underline" onClick={onClick}>{title}</h4>
